 ...
 */
public class NSGAIISchedConv extends Algorithm {
   ...
   */
  public NSGAIISchedConv(Problem problem) {
...
//	    } //for       
	   ...
	   // NEW STEP FOR THE CONVERGENCE STUDY: for calculating the convergence speed of the front in terms of the quality indicators
			if (indicatorList.length > 0) {
				QualityIndicator indic;
				// System.out.println("PF file: " +
				// paretoFrontFile_[problemId]);
				indic = new QualityIndicator(problem_,
						paretoFrontFile);

				RankingNonRepeatedSols rank = new RankingNonRepeatedSols(population);
			    SolutionSet archive = rank.getSubfront(0);
			    
				for (int j = 0; j < indicatorList.length; j++) {
					if (indicatorList[j].equals("HVNonNormalized")) {
						double value = indic.getHypervolumeNonNormalized(archive);
						metrics[problemID][inst][alg][j][generations] += value;
					}
					if (indicatorList[j].equals("SPREADNonNormalized")) {
							double value = indic.getSpreadNonNormalized(archive);
							metrics[problemID][inst][alg][j][generations] += value;
					}
					if (indicatorList[j].equals("IGDNonNormalized")) {
							double value = indic.getIGDNonNormalized(archive);
							metrics[problemID][inst][alg][j][generations] += value;
					}
					if (indicatorList[j].equals("EPSILON")) {
							double value = indic.getEpsilon(archive);
							metrics[problemID][inst][alg][j][generations] += value;
					}
				} // for
			} // if
...
//		} // if
	    ...
	    // Return the first non-dominated front
	    RankingNonRepeatedSols ranking = new RankingNonRepeatedSols(population);
	    return ranking.getSubfront(0);
	  } // execute
