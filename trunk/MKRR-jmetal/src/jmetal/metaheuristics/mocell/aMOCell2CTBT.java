 ...
 */
public class aMOCell2CTBT extends Algorithm{
   ...
   */
  public aMOCell2CTBT(Problem problem){
    problem_ = problem;
  } // aMOCell2
  ...
  public SolutionSet execute() throws JMException {
    ...
    while (evaluations < maxEvaluations){                                 
      for (int ind = 0; ind < currentSolutionSet.size(); ind++){
        ...
        //parents
        parents[0] = individual;//(Solution)selectionOperator.execute(neighbors[ind]);
        if (archive.size() > 0) {
      ...
      if (evaluations <= 2*currentSolutionSet.size())
    	  archive.printObjectivesToFile("Archive1stGen.pf");
    }
    return archive;
  } // execute     
} // aMOCell2
