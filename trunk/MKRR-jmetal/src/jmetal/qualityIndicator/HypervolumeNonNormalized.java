 ...
 *           pp. 257-271, 1999.
 *           
 *  Te difference with Hypervolume is that in this case we are not normalizing the
 *  Pareto front with respect to the true Pareto front, why? because when the Pareto
 *  front is not good enough, the normalization method is not giving numbers between 0 and 1
 *  
 */
public class HypervolumeNonNormalized {
  ...
  */
  public HypervolumeNonNormalized() {
  ...
  double calculateHypervolume(double [][] front, int  noPoints,int  noObjectives){
    ...
    while (n > 0) {
      ...
      if (noObjectives < 3) {
        ...
        tempVolume = Math.abs(front[0][0]);
      } else
        tempVolume = calculateHypervolume(front,
                                          noNondominatedPoints,
                                          noObjectives - 1);
      
      tempDistance = Math.abs(surfaceUnchangedTo(front, n, noObjectives - 1));
...
//    minimumValues = utils_.getMinimumValues(paretoTrueFront,numberOfObjectives);
    ...
    // We don't normalize in this case
    ...
    // The inverted front is computed in this non-normalized case by negating the two objectives
    invertedFront = utils_.inverted(paretoFront);
