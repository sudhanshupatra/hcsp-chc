...
public class QualityIndicator {
  ...
  public QualityIndicator(Problem problem, String paretoFrontFile, boolean normalized) {
	  if (normalized) {  
		problem_ = problem ;
	    utilities_ = new jmetal.qualityIndicator.util.MetricsUtil() ;
	    trueParetoFront_ = utilities_.readNonDominatedSolutionSet(paretoFrontFile);
	    trueParetoFrontHypervolume_ = new Hypervolume().hypervolume(
		    ...
		    utilities_ = new jmetal.qualityIndicator.util.MetricsUtil() ;
		    trueParetoFront_ = utilities_.readNonDominatedSolutionSet(paretoFrontFile);
		    trueParetoFrontHypervolume_ = new HypervolumeNonNormalized().hypervolume(
		                 trueParetoFront_.writeObjectivesToMatrix(),   
		                 trueParetoFront_.writeObjectivesToMatrix(),
		                 problem_.getNumberOfObjectives());
		  }
   ...
   * @return The value of the hypervolume indicator
   */
  public double getHypervolumeNonNormalized(SolutionSet solutionSet) {
    return new HypervolumeNonNormalized().hypervolume(solutionSet.writeObjectivesToMatrix(),
                                         trueParetoFront_.writeObjectivesToMatrix(),
                                         problem_.getNumberOfObjectives());
  } // getHypervolume
  ...
  public double getIGDNonNormalized(SolutionSet solutionSet) {
	    return new InvertedGenerationalDistanceNonNormalized().invertedGenerationalDistance(
	                    solutionSet.writeObjectivesToMatrix(),
	                    trueParetoFront_.writeObjectivesToMatrix(),
	                    problem_.getNumberOfObjectives());
	  } // getIGD
 ...
 /**
  ...
  public double getSpreadNonNormalized(SolutionSet solutionSet) {
	    return new SpreadNonNormalized().spread(solutionSet.writeObjectivesToMatrix(),
	                               trueParetoFront_.writeObjectivesToMatrix(),
	                               problem_.getNumberOfObjectives());
	  } // getGD
