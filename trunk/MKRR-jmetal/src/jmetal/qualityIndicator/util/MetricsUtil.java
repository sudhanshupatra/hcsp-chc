    ...
    for (int i = 0; i < front.length;i++) {
      ...
      for (int j = 0; j < front[i].length; j++) {
        normalizedFront[i][j] = (front[i][j] - minimumValue[j]) /
    	  ...
    	  // Modified by Bernbe Dorronsoro
//    	  normalizedFront[i][j] = Math.abs((Math.abs(front[i][j]) - Math.abs(minimumValue[j])) /
//    	  							(Math.abs(maximumValue[j]) - Math.abs(minimumValue[j])));
      }
    }
  ...
  public double[][] inverted(double [][] front) {
	    double [][] invertedFront = new double[front.length][];
