/**
 * Contains some useful classes to organizate Solution and solutionSet
 */
...
public class AdaptiveGrid {
	...
	private int      bisections_    ; ///< Number of bi-divisions of the objective space
	private int      objectives_    ; ///< Objectives of the problem
	private int[]    hypercubes_    ; ///< Number of solutions into a specific hypercube in the adaptative grid
	private double[] lowerLimits_   ; ///< Grid lower bounds
	private double[] upperLimits_   ; ///< Grid upper bounds
	private double[] divisionSize_  ; ///< Size of hypercube for each dimension
	private int      mostPopulated_ ; ///< Hypercube with maximum number of solutions
	private int[]    occupied_      ; ///< Indicates when an hypercube has solutions
	...
	public AdaptiveGrid( int bisections, int objetives ) {
		...
		int hypercubesLength = (int) Math.pow( 2.0 , bisections_*objectives_ );
		hypercubes_          = new int[ hypercubesLength ];

		for (int i = 0 ; i < hypercubesLength ; i++ )
			hypercubes_[i] = 0;
	} //AdaptativeGrid
	...
	public int rouletteWheel(){
		...
		//Calculate a random value between 0 and inverse sum
