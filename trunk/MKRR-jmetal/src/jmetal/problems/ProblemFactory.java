	...
	public Problem getProblem(String name, Object[] params) throws JMException {
		...
		if (name.substring(0, name.length() - 1).equalsIgnoreCase("DTLZ"))
			...
			base += "cec2009Competition.";
				...
				"CEC2009"))
			base += "cec2009Competition.";
		else if (name.equalsIgnoreCase("scheduling.u_i_hihi"))
			base += "";
		else if (name.equalsIgnoreCase("scheduling.u_i_hihi_mak_flow"))
			base += "";
		else if (name.equalsIgnoreCase("scheduling.u_i_lolo"))
			base += "";
		else if (name.toLowerCase().startsWith("scheduling."))
			// Ignoro todos los que arrancan con "scheduling.".
			base += "";
...
//			Problem problem = new jmetal.problems.scheduling.MO_MKRR("Int");
			return problem;
		}// try
		catch (Exception e) {
