	...
	do
		...
		do
			...
			do
				...
				echo "scripts_calibracion/${CfgFile}" > Config_LAN4_cal2.cfg
				echo "${DataFile}" >> Config_LAN4_cal2.cfg
				echo "${OutputFile}.sol" >> Config_LAN4_cal2.cfg
				
				time($EXEC Config_LAN4_cal2.cfg > ${OutputFile}.log)
			done
		done
	done
done
