...
void Diverge::diverge(Solution& sol) const {
	...
	for (int i = 0; i < MUT_MAQ; i++) {
		...
		while (!modificado) {
			...
			for (int auxMachineId = 0; auxMachineId < sol.machines().size(); auxMachineId++) {
				if ((auxMachineId != machineId) && (rand01() >= 0.5)
						&& (sol.machines()[auxMachineId].countTasks() == 0)
						&& (sol.machines()[machineId].countTasks() > 0)) {

					int bestTaskPosForMachine;
					bestTaskPosForMachine
							= sol.getMinDestinationCostTaskPosByMachine(
									machineId, auxMachineId);

					if (bestTaskPosForMachine >= 0) {
						int bestTaskIdForMachine;
						bestTaskIdForMachine
								= sol.machines()[machineId].getTask(
										bestTaskPosForMachine);

						sol.removeTaskAt(machineId, bestTaskPosForMachine);
						sol.addTask(bestTaskIdForMachine, auxMachineId);
			...
			if ((rand01() >= 0.5) && (sol.machines()[machineId].countTasks()
					> 0)) {
				...
				cout << endl << "countTasks: " << costsByTaskPos.size() << endl;
				...
				cout << endl << "taskPos: " << taskPos << endl;
				...
				if (sol.machines()[minCostMachineId].countTasks() > 0) {
					...
					cout << "minCostMachineId: " << minCostMachineId << endl;
					cout << "minCostTaskPosOnMachine: " << minCostTaskPosOnMachine << endl;
			...
			if ((rand01() >= 0.5) && (sol.machines()[machineId].countTasks() > 0)) {
				// Se selecciona una tarea T según rueda de ruleta por su COSTO y se
...
_sc.set_contents_state_variable("_diverge_probability", (char *) probability,
	1, sizeof(float));
}

void Diverge::UpdateFromState(const StateCenter& _sc) {
unsigned long nbytes, length;
_sc.get_contents_state_variable("_diverge_probability", (char *) probability,
	nbytes, length);
}
...
_number_operator(_number_op), direction(dir), migration_rate(1),
	migration_size(1), migration_selection_1(0), migration_selection_2(0),
	migration_selection_conf_1(0), migration_selection_conf_2(0) {
}
...
_sc.set_contents_state_variable("_migration_rate", (char *) &migration_rate, 1,
	sizeof(migration_rate));
_sc.set_contents_state_variable("_migration_size", (char *) &migration_size, 1,
	sizeof(migration_size));
...
_sc.set_contents_state_variable("_migration_selection_conf_1",
	(char *) &migration_selection_conf_1, 1, sizeof(migration_selection_conf_1));
_sc.set_contents_state_variable("_migration_selection_conf_2",
	(char *) &migration_selection_conf_2, 1, sizeof(migration_selection_conf_2));
}
...
for (int i = 0; i < migration_size; i++) {
	...
	solution_to_send = pop.parents()[pop.setup().pool().selector(
			migration_selection_1).select_one(pop.parents(), pop.offsprings(),
			pop.fitness_values(), migration_selection_conf_1, false).index];
...
if (synchronized) // synchronous mode: blocked until data are received
{
	...
	for (int i = 0; i < migration_size; i++) {
		...
		ind = pop.setup().pool().selector(migration_selection_2).select_one(
				pop.parents(), pop.offsprings(), pop.fitness_values(),
		...
		// remplace policy
		if ((solution_received->fitness() <= solution_to_remplace->fitness()
				&& direction == minimize) || (solution_received->fitness()
				>= solution_to_remplace->fitness() && direction == maximize)) {
...
os << "Migration." << endl << "\t" << " Rate: " << migration.migration_rate
	...
	<< " Selection 1: " << migration.migration_selection_1 << endl << "\t"
	<< " Selection 1 Parameter: " << migration.migration_selection_conf_1
...
const Rarray<struct individual>& fitness_values, const unsigned int dummy,
const bool remplace) const { // select a random individual
...
const Rarray<Solution*>& to_select_1, const Rarray<Solution*>& to_select_2,
...
const Rarray<Solution*>& to_select_1, const Rarray<Solution*>& to_select_2,
const Rarray<struct individual>& fitness_values, const unsigned int portion,
const bool remplace) const {
...
return fitness_values[rand_int(0, ((fitness_values.size() * new_portion) / 100)
	- 1)];
}
...
const Rarray<Solution*>& to_select_1, const Rarray<Solution*>& to_select_2,
const Rarray<struct individual>& fitness_values, const unsigned int position,
const bool remplace) const {
...
const Rarray<Solution*>& to_select_1, const Rarray<Solution*>& to_select_2,
const Rarray<struct individual>& fitness_values, const unsigned int param,
const bool remplace) const {
...
_sc.set_contents_state_variable("_parameter_select_new_pop", (char *) &r, 1,
	sizeof(float));
}
...
problem(pbm), params(setup), _stat(), _userstat(), _sc(), current_population(
	...
	(-1) * best_cost), best_solution(problem), average_cost(0.0),
	standard_deviation(0.0), time_spent_in_trial(0.0), total_time_spent(0.0),
	start_trial(0.0), start_global(0.0), _current_trial("_current_trial", _sc),
	_current_iteration("_current_iteration", _sc), _current_evaluations(
			...
			"_best_sol_trial", _sc), _best_cost_trial("_best_cost_trial", _sc),
	...
	_time_best_found_trial("_time_best_found_trial", _sc), _time_spent_trial(
			"_time_spent_trial", _sc), _trial_best_found("_trial_best_found",
			_sc), _iteration_best_found("_iteration_best_found", _sc),
	_evaluations_best_found("_evaluations_best_found", _sc),
	_global_best_solution("_global_best_solution", _sc), _global_best_cost(
			"_global_best_cost", _sc), _global_worst_cost("_global_worst_cost",
			_sc), _time_best_found("_time_best_found", _sc),
	_crossover_probability("_crossover_probability", _sc),
	_diverge_probability("_diverge_probability", _sc), _migration_rate(
			"_migration_rate", _sc), _migration_size("_migration_size", _sc),
	...
	_select_parents("_select_parents", _sc), _select_offsprings(
			...
			"_parameter_select_new_pop", _sc), _display_state("_display_state",
			_sc) {
current_trial(0);
current_iteration(0);
...
_sc.set_contents_state_variable(aux, (char *) new_probability, MAX_PROB_PER_OP,
	sizeof(float));
}
...
_sc.set_contents_state_variable("_select_offsprings", (char *) &selection, 1,
	sizeof(int));
}

void Solver::parameter_select_new_pop(const float value) {
_sc.set_contents_state_variable("_parameter_select_new_pop", (char *) &value,
	1, sizeof(float));
}
...
case minimize:
betterG = (best_cost < global_best_cost() || (best_cost == global_best_cost()
		&& time_spent_in_trial < time_best_found()));
worseG = (worst_cost > global_worst_cost());
betterT = (best_cost < best_cost_trial() || (best_cost == best_cost_trial()
...
StartUp();
while ((current_iteration() < nb_generations) && !(terminateQ(problem, *this,
	params)))
DoStep();
...
_netstream << pack_begin << _mypid << current_trial() << current_iteration()
	<< current_evaluations() << best_cost_trial() << best_solution_trial()
	<< iteration_best_found_in_trial() << evaluations_best_found_in_trial()
	<< time_best_found_trial() << worst_cost_trial() << current_best_cost()
	<< current_best_solution() << current_worst_cost()
	<< current_average_cost() << current_standard_deviation() << pack_end;
}
...
_netstream << pack_begin >> r_pid >> _current_trial >> _current_iteration
	...
	>> _time_best_found_in_trial >> _worst_cost_trial >> best_cost
	>> best_solution >> worst_cost >> average_cost >> standard_deviation
	<< pack_end;
return r_pid;
}
...
KeepHistory(_best_solution_trial, _best_cost_trial, _worst_cost_trial,
		_time_best_found_in_trial, start_global + _time_best_found_in_trial);
...
StartUp();
if (mypid != 0) {
while (!final_phase && (current_iteration() < nb_generations) && !(terminateQ(
		problem, *this, params)))
	DoStep();
