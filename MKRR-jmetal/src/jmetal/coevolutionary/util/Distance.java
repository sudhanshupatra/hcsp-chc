...
import jmetal.coevolutionary.base.operator.comparator.ObjectiveComparator;
...
public class Distance {      
	 ...
	 * <code>SolutionSet</code>. This matrix is simmetric.
	...
	public double[][] distanceMatrix(SolutionSet solutionSet) {
		...
		for (int i = 0; i < solutionSet.size(); i++){
			...
			// It's not necessary obtain the whole solution
	 ...
	 * @param solution The <code>Solution</code>, must be the extended version.
	 * @param solutionSet The <code>SolutionSet</code>.
	...
	public double distanceToSolutionSet( Solution    solution    ,
		...
		int loadingPosition = solutionSet.getLoadingPosition();
		while( it.hasNext() ){
			double aux = this.distanceBetweenSolutions( solution , new Solution( it.next() , loadingPosition ) );
			if (aux < distance)
				distance = aux;
		} // while
	...
	public void crowdingDistanceAssignment( SolutionSet solutionSet , int nObjs ) {
		...
		SolutionSet front = new SolutionSet( solutionSet.getLoadingPosition()   ,
				                             solutionSet.getNumberOfIslands()   ,
				                             solutionSet.getNumberOfSolutions() ,
				                             size );
		front.setBestExternResults( solutionSet );
