 ...
 */
public class SpreadNonNormalized {
...
//    int numberOfTruePoints = normalizedParetoFront.length;
    ...
    // the metric)
    double df = utils_.distance(front[0],trueParetoFront[0]);
    double dl = utils_.distance(front[front.length-1],
    		       trueParetoFront[trueParetoFront.length-1]);
    ...
    for (int i = 0; i < (front.length-1); i++) {
      mean += utils_.distance(front[i],front[i+1]);
    } // for
    ...
    if (numberOfPoints > 1) {
      for (int i = 0; i < (numberOfPoints -1); i++) {
        diversitySum += Math.abs(utils_.distance(front[i],
        		                 front[i+1]) - mean);
      } // for
