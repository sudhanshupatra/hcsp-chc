 ...
 */
public class PaperCEC10StudyMOCell extends ExperimentNoPareto {

	private int independentRunsDf_ = 100; // Number of independent runs per
											// algorithm and problem
	// private int independentRunsDf_ = 1; // Number of independent runs per
	// algorithm and problem
	private int numberOfThreadsDf_ = 1; // Number of threads to use (= number of
										// algorithms to run in parallel)
	// private int numberOfInstancesDF_ = 2; // Number of instances to solve per
	// problem
	private int numberOfInstancesDF_ = 100; // Number of instances to solve per
											// problem
	...
	public synchronized void algorithmSettings(Problem problem,
		...
		try {
			...
			for (int i = 0; i < numberOfAlgorithms; i++) {
				...
				parameters[i].setProperty("MUTATION_PROBABILITY", new Double(
						1.0 / problem.getNumberOfVariables()).toString());
				// parameters[i].setProperty("MUTATION_PROBABILITY", "0.75");
			...
			// for (int i = 0; i < numberOfAlgorithms; i++)
			// algorithm[i] = new
			// NSGAII_Settings(problem).configure(parameters[i]);
	...
	public static void main(String[] args) throws JMException, IOException {
		...
		PaperCEC10StudyMOCell exp = new PaperCEC10StudyMOCell(); // exp =
																	// experiment

		exp.experimentName_ = "PaperCEC10StudyMOCell";
