...
public class MOCellReprInt_main {
...
//    } // else
    ...
    if (args.length != 4) {
		System.out.println("Error. Try: MOCellReprInt_main <seed> <density> <surface> <maxBypassLinks>");
		System.exit(-1);
    } // if

	problem = new MOBypassLinks(new Integer(args[0]).intValue(), new Integer(args[1]).intValue(), new Integer(args[2]).intValue(), new Integer(args[3]).intValue()); // seed, density, surface
	algorithm = new aMOCell4(problem);
	//algorithm = new aMOCell4CTBT(problem);
    //algorithm = new sMOCell4CTBT(problem);
    ...
    //mutation = MutationFactory.getMutationOperator("IntegerMutation");                    
