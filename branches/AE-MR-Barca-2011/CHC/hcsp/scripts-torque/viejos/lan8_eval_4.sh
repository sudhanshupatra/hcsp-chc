...
# Requerimientos
#PBS -l nodes=1:cpu8:ppn=8,walltime=20:00:00
...
do
	...
	OutputFile="evaluacion/lan8/4096x128/${data[i]}"
	
	echo "Datos $DataFile"
	echo "CfgFile $CfgFile"
	cat $CfgFile
	
	echo "${CfgFile}" > Config_LAN8_eval4.cfg
	echo "$DataFile" >> Config_LAN8_eval4.cfg
	echo "$OutputFile.sol" >> Config_LAN8_eval4.cfg
	
	time($EXEC Config_LAN8_eval4.cfg > $OutputFile.log) 
done