...
# Output path
#PBS -e /home/siturria/hcsp-chc/branches/AE-MF-Barca-2011/CHC/hcsp/ejecuciones/evaluacion/lan8+8y/
#PBS -o /home/siturria/hcsp-chc/branches/AE-MF-Barca-2011/CHC/hcsp/ejecuciones/evaluacion/lan8+8y/
...
do
    ...
    do
        ...
        do
                ...
                OutputPath="${BASE_PATH}/ejecuciones/evaluacion/lan8+8y/${cfg[c]}/${data[i]}/${j}"
                ...
                echo "${CfgFile}" > Config_LAN8+8y_eval.cfg
                echo "${DataFile}" >> Config_LAN8+8y_eval.cfg
                echo "${OutputPath}/${j}.sol" >> Config_LAN8+8y_eval.cfg
                echo "${PesosFile}" >> Config_LAN8+8y_eval.cfg
                echo "${dim_tasks[c]}" >> Config_LAN8+8y_eval.cfg
                echo "${dim_machines[c]}" >> Config_LAN8+8y_eval.cfg
            
                time($EXEC Config_LAN8+8y_eval.cfg > ${OutputPath}/${j}.log) 
                echo "==============================================="
        done
    done
done
