...
# Requerimientos
#PBS -l nodes=2:class1:ppn=8,walltime=40:00:00
...
# Output path
#PBS -e /home/siturria/hcsp-chc/branches/AE-MF-Barca-2011/CHC/hcsp/ejecuciones/evaluacion/lan8+8x/
#PBS -o /home/siturria/hcsp-chc/branches/AE-MF-Barca-2011/CHC/hcsp/ejecuciones/evaluacion/lan8+8x/
...
do
    ...
    do
        ...
        do
                ...
                OutputPath="${BASE_PATH}/ejecuciones/evaluacion/lan8+8x/${cfg[c]}/${data[i]}/${j}"
                ...
                echo "${CfgFile}" > Config_LAN8+8x_eval.cfg
                echo "${DataFile}" >> Config_LAN8+8x_eval.cfg
                echo "${OutputPath}/${j}.sol" >> Config_LAN8+8x_eval.cfg
                echo "${PesosFile}" >> Config_LAN8+8x_eval.cfg
                echo "${dim_tasks[c]}" >> Config_LAN8+8x_eval.cfg
                echo "${dim_machines[c]}" >> Config_LAN8+8x_eval.cfg
            
                time($EXEC Config_LAN8+8x_eval.cfg > ${OutputPath}/${j}.log) 
                echo "==============================================="
        done
    done
done
